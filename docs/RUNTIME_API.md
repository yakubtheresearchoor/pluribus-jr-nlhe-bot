...
| `game.json` (in `CONN_BP`/`BP_ROOT` dir) | _(v1 fallback)_ | **Per-blueprint game economics manifest**: `{num_players, sb, bb, ante, stack, rake_rate, rake_cap, no_flop_no_drop}` (units). Loaded at startup and installed process-wide — every real-time solve builds seam trees and rakes terminals under the SAME game class its blueprint was solved with (stakes vary caps/antes/depth; the 50bb blueprint carries `stack:100`). Legacy dirs without one fall back to production v1 with a startup WARNING. The blueprint writer emits it automatically. |
