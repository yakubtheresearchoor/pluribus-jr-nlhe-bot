...
impl ChanceTable {
    ...
    /// Build a RIVER-START chance table: the full 5-card board is known, so there
    /// is NO chance to come — `hand_ranks_base` holds each hand's FINAL 7-card rank
    /// and the chance dimension is empty. Paired with `TurnStartGame` over a river
    /// seam tree (no chance node), the evaluate_terminal "None" branch does an exact
    /// HU showdown on these final ranks (sorted via `sorted_opp_arrays`). This is the
    /// river-rooted exact solver's table — same showdown machinery, board complete.
    pub fn compute_river_start(known_board: &[Card], ranges: &[Vec<f32>], num_players: u8) -> Self {
        assert_eq!(known_board.len(), 5, "river start needs exactly 5 known board cards");
        let board_set: Vec<u8> = known_board.iter().map(|&c| c as u8).collect();
        ...
        // FINAL 7-card rank (2 hole + 5 board) — the showdown strength, no more cards.
        let mut hand_ranks_base = vec![0u16; num_valid];
        let mut hand_cards = vec![0u8; num_valid * 2];
        for (i, &hi) in valid_hand_indices.iter().enumerate() {
            let (c1, c2) = index_to_card_pair(hi as usize);
        ...
        let nc = if num_players == 2 {
            let (w0, w1) = (&initial_weights[0], &initial_weights[1]);
            let mut nc = 0.0f64;
            for h0 in 0..num_valid {
                let mask0: u64 = (1u64 << hand_cards[h0 * 2]) | (1u64 << hand_cards[h0 * 2 + 1]);
                for h1 in 0..num_valid {
        ...
        // No chance to come: empty river dimension.
        ChanceTable {
            hand_ranks_base,
            valid_hand_indices,
            num_valid,
            ...
            chance_ranks_table: Vec::new(),
            chance_sorted_strength: Vec::new(),
            chance_sorted_indices: Vec::new(),
            initial_weights,
            num_players,
            num_combinations: nc,
        }
