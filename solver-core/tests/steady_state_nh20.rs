// nh=20 steady-state — third data point to verify the 1.22 scaling
// exponent is not a two-point artifact stretched 3x beyond its range.
// Pairs with nh=12 (45.5s) and nh=16 (64.6s) to fit a 3-point exponent
// or expose curvature.
...
#[test]
#[ignore = "Third data point: confirm nh=12→nh=16→nh=20 exponent."]
fn steady_state_nh20_20iters() {
    let nh = 20usize;
    let n_iters = 20usize;
    eprintln!("\n=== Steady-state nh={} over {} iters (3rd data point for exponent confirmation) ===", nh, n_iters);
    ...
    // 3-point analysis: nh=12 → nh=16 → nh=20
    let nh12_late = 45.47f64;
    let nh16_late = 64.6f64;
    let nh20_late = late_mean;
    let exp_12_16 = (nh16_late / nh12_late).ln() / (16.0_f64 / 12.0).ln();
    let exp_16_20 = (nh20_late / nh16_late).ln() / (20.0_f64 / 16.0).ln();
    let exp_12_20 = (nh20_late / nh12_late).ln() / (20.0_f64 / 12.0).ln();
    eprintln!("=== 3-point scaling exponent analysis (corrected tree) ===");
    eprintln!("  nh=12 late-half: {:.1} s", nh12_late);
    eprintln!("  nh=16 late-half: {:.1} s", nh16_late);
    eprintln!("  nh=20 late-half: {:.1} s", nh20_late);
    eprintln!();
    eprintln!("  exponent nh=12 → nh=16: {:.2}", exp_12_16);
    eprintln!("  exponent nh=16 → nh=20: {:.2}", exp_16_20);
    eprintln!("  exponent nh=12 → nh=20: {:.2} (overall)", exp_12_20);
    if (exp_12_16 - exp_16_20).abs() < 0.3 {
        eprintln!("  → Exponent is STABLE across the range. Trust the extrapolation.");
    } else {
        eprintln!("  → Exponent SHIFTS — scaling is curved. nh=50 extrapolation is uncertain.");
    }
    eprintln!();
    eprintln!("=== nh=50 baseline projection (3 anchored exponents) ===");
    for &(label, exp) in &[("anchored on nh=12→16", exp_12_16), ("anchored on nh=16→20", exp_16_20), ("anchored on nh=12→20", exp_12_20)] {
        let proj = nh20_late * (50.0_f64 / 20.0).powf(exp);
        eprintln!("  {}: {:.0} s/iter → 200 iters = {:.1} h = {:.2} d",
            label, proj, proj * 200.0 / 3600.0, proj * 200.0 / 86400.0);
    }
}
