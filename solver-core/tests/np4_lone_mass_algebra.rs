...
/// UNIFORM importance sampling for the MC outer (no CDF: sample g0 uniform,
/// weight r0[g0]·nh/M). Kernel-simpler than CDF-weighted — this gates whether
/// its variance is acceptable vs the validated weighted estimator.
#[test]
fn k4_mc_uniform_variance() {
    let deck_n = 12usize;
    let hands = make_hands(deck_n as u8);
    let nh = hands.len();
    let mut rng = Lcg(0x0417F);
    let mk = |rng: &mut Lcg| -> Vec<f64> {
        (0..nh).map(|_| if rng.f() < 0.25 { 0.0 } else { rng.f() }).collect()
    };
    let (r0, r1, r2, r3) = (mk(&mut rng), mk(&mut rng), mk(&mut rng), mk(&mut rng));
    let roles = cf::roles(&hands, &r1, &r2, &r3);
    let agg = cf::build(&roles, &hands);
    let (mut cvw, mut cvu) = (0.0f64, 0.0f64);
    for h in (0..nh).step_by(13) {
        let truth = mass4_closed_inner(&hands, h, &r0, &roles, &agg);
        if truth.abs() < 1e-9 { continue; }
        let hh = hands[h];
        let m_draws = 128usize;
        let mut est = |uniform: bool, seed: u64| -> f64 {
            if !uniform { return mass4_mc(&hands, h, &r0, &roles, &agg, m_draws, seed); }
            let mut r = Lcg(seed);
            let mut acc = 0.0;
            for _ in 0..m_draws {
                let g0 = (r.f() * nh as f64) as usize % nh;
                let (a, b) = hands[g0];
                if r0[g0] == 0.0 || !disjoint((a, b), hh) { continue; }
                acc += r0[g0] * cf::mass3_closed(&roles, &hands, &agg, &[hh.0, hh.1, a, b]);
            }
            acc * nh as f64 / m_draws as f64
        };
        for (uni, cv) in [(false, &mut cvw), (true, &mut cvu)] {
            let d: Vec<f64> = (0..192).map(|s| est(uni, 0xABCD + s)).collect();
            let mean = d.iter().sum::<f64>() / d.len() as f64;
            let var = d.iter().map(|x| (x - mean).powi(2)).sum::<f64>() / d.len() as f64;
            *cv = cv.max(var.sqrt() / truth);
            assert!((mean - truth).abs() / truth < 0.05, "biased? uni={uni} h={h}");
        }
    }
    eprintln!("MC M=128 CV: weighted={cvw:.3} uniform={cvu:.3}");
}
