#![cfg(feature = "metal")]
// Progressive GPU 50-hand 3-player convergence test, hitting checkpoints
// against the CPU's brute-force reference trajectory.
...
#[test]
fn gpu_50hand_progressive_convergence() {
    ...
    // Granular checkpoints to track CPU reference:
    // CPU reference: iter 100=2.57%, 500=0.13%, 1000=0.05%, 2000=0.01%
    let checkpoints: &[u32] = &[10, 25, 50, 100, 200, 500, 1000, 2000];
    let mut prev = 0u32;

    eprintln!("\n=== GPU 50-hand 3-player progressive convergence ===");
    eprintln!("CPU reference: iter 100=2.57%, 500=0.13%, 1000=0.05%, 2000=0.01%");
    let t0 = Instant::now();
    ...
    for &cp in checkpoints {
        ...
        eprintln!(
            "GPU iter {:5}: {:.4}% of pot  (iter-batch took {:?}, total {:?})",
            cp, pct, it_elapsed, total_elapsed
        );
        // Flush so each checkpoint visible immediately
