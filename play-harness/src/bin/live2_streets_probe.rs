...
fn main() {
    ...
    // ── TURN ROOT: 4-card board → river chance (47) + river betting → showdown ──
    {
        ...
        let nh = table.num_valid;
        let game = TurnStartGame::new(table);
        let tree = build_tree(&spec.street_seam_config(BoardState::Turn, 2, commit, pot, live2_bet_menu())).unwrap();
        let t = Instant::now();
        let mut cfr = VectorCfr::new(&tree, vec![nh; 2]);
        cfr.run(&tree, &game, iters);
        let run_ms = t.elapsed().as_millis();
        ...
        let sum: f32 = probs.iter().sum();
        println!(
            "TURN   nh={nh} nodes={} na={na} | table {}ms + solve {}ms ({iters} it) | hand0 Σp={:.3} {:?}",
            tree.num_nodes(), tab_ms, run_ms, sum,
            probs.iter().map(|p| (p * 100.0).round() / 100.0).collect::<Vec<_>>()
        );
    }
